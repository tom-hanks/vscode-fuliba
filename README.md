...
- **支持楼主**：就是论坛里那个张国立竖大拇指的按钮，现在楼主楼底下直接能点，赞数和论坛实时同步。走的是论坛自己的评分接口，所以「不能给自己发表的帖子评分」「已经支持过」这些限制跟在网页上一模一样
...
- **搜索**：在 VS Code 里直接搜帖子
- **最新福利（门户）**：福利吧首页的文章流直接挂在版块树最末一项，展开就是文章列表（无封面，每行「日期 · 阅读 · 评论」），与版块共用翻页。游客可读、无需 Cookie，但不能评论或点赞——网页上的限制在这里一模一样
...
左侧活动栏点福利吧图标，展开版块 → 点帖子标题即可在编辑器里打开详情。**版块树最末一项是「最新福利」**，展开就是福利吧首页的文章列表（游客可读、无需登录），点文章标题打开详情看正文与评论区。
...
转码产物是个本地文件，按理说直接给播放器一个 `file+.vscode-resource...` 地址就行 —— 但这条路在帖子面板里走不通：
VS Code 的 webview 资源代理读不出它，播放器只会报 `MEDIA_ELEMENT_ERROR: Format error`。
所以扩展在打开含可修复视频的帖子时，会**在自己进程里起一个只监听 `127.0.0.1` 的 HTTP 服务**专门送这些文件，
播放器从它取流，绕开资源代理。

这个服务只做几件事，边界很窄：

- 只绑 `127.0.0.1`（外网和局域网都连不上），端口由系统随机分配
- 地址里带一段随机 token，别的程序猜不到；只放行缓存目录里那几种 `.mp4` 文件名（防路径穿越）
- 支持 `Range` / `206`（大文件要能边下边播）、拒绝越界范围、不给缓存
- 扩展停用时自动关闭
...
- 修视频音轨用的那个本地媒体服务只监听 `127.0.0.1`，只送扩展自己转出来的缓存文件，不代理也不上传任何东西（详见[上一节](#为什么转好的文件要经由本地服务送出去)）
- 本扩展没有遥测，不收集任何数据
- 仓库源码里不存在任何硬编码的凭据
