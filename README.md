...
- **视频内嵌**：B 站等白名单站点的播放器直接嵌进详情页，点开就能播（只认白名单域名，其它一律不嵌）。播放器下面有一条「没声音」的说明，原因见下方[关于声音](#关于视频没有声音)
...
这是 VS Code 的限制，不是解析漏了。webview 跑在 Electron 自带的 Chromium 上，而那份构建按专利授权要求裁掉了 **AAC 解码器**（[microsoft/vscode#167685](https://github.com/microsoft/vscode/issues/167685)；官方文档里 webview 只保证 Wav / Mp3 / Ogg / Flac 音轨）。B 站的音轨、论坛直传 mp4 的音轨基本都是 AAC，于是画面正常、声音全无——音轨恰好是 MP3 的视频则一切正常。

给 `iframe` 加 `allow="autoplay"` 解决不了这个：那是权限问题，不是编解码问题。

所以每个播放器下面都挂了一条说明，点「在浏览器里听」可以直接跳到 B 站视频页或视频文件本身，用系统浏览器看。要在编辑器里听 AAC 音轨，得把视频音轨抽出来转成 MP3 再和静音画面同步播放（有扩展是这么做的），代价是要装 ffmpeg——对一个摸鱼插件来说不划算。
